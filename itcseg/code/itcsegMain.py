...
parser.add_argument("--mode", default="prepare", type=str, nargs="+", help="Choose your mode: train/test/apply")
# parser.add_argument("--mode", default="train", type=str, help="Choose your mode: train/test/apply")
# parser.add_argument("--mode", default="test", type=str, help="Choose your mode: train/test/apply")
# parser.add_argument("--mode", default="apply", type=str, nargs="+", help="Choose your mode: train/test/apply")
    ...
    def validate(self):
        ...
        for i, (nb_idxs, nb_vals, nb_anns, nb_pcd_idxs, nb_inverse_idxs, fname) in enumerate(self.val_loader):
            ...
            if self.logger is not None:
                self.logger.write(f"%s ---> Start validating %s (OA, IoU[other], IoU[conf], mIoU):\t" % (datetime.datetime.now(), fname))
...
class TrainDataGenerator(DataGenerator):
    ...
    def read_all_data(self, myLogger):
        ...
        for fid, pcd_fname in enumerate(self.pcd_fnames):
            las = laspy.open(glob.glob(os.path.join(self.data_dir, pcd_fname + '_cfd.la*'))[0]).read()
            assert hasattr(las,'intensity')
            assert hasattr(las,'number_of_returns')
            assert hasattr(las,'VegCls'), "las file should have a scalar field called 'VegCls' from previous vegcls module with vegetation/tree points 2 and other points 1"
            assert hasattr(las, 'itc_ref'), "las file should have manually added scalar field called 'itc_ref' as the reference tree ID"
            ...
            columns = np.transpose(np.array([las.x, las.y, las.z, las.intensity, las.number_of_returns,las.itc_ref, las.conf]))  # las.point_format.dimension_names
            abg_ind = columns[:, -2] > 0
            conf_ind = columns[:, -1] > 0.75
            sel_ind = (abg_ind & conf_ind)
            columns[sel_ind, -1] = 2.0  # target class
            columns[~sel_ind, -1] = 1.0  # else
...
def gau_interp3D(delta_dist,band_width=3.0):
    gau_kernel = 1/np.sqrt(2*np.pi)/band_width*np.exp(-(delta_dist ** 2) / 2 / band_width / band_width)
    return gau_kernel
def assign_confidence(configs, pcd_fnames):
    data_dir = configs["dataset"]["data_path"]
    gau_bandwidth = configs["dataset"]["gau_bandwidth"]
    normalize_intensity = configs["dataset"]["normalize_intensity"]

    total_seg_counts = 0
    for fid, pcd_fname in enumerate(pcd_fnames):
        las = laspy.open(glob.glob(os.path.join(data_dir, pcd_fname + ".la*"))[0]).read()
        columns = np.transpose(np.array([las.x, las.y, las.z, las.intensity, las.itc_ref]))  # las.point_format.dimension_names
        columns[:, 3] = columns[:, 3] / normalize_intensity

        columns[:, :3] = columns[:, :3] - np.min(columns[:, :3], axis=0)
        abg_ind = columns[:, -1] > 0

        abg_pts = columns[abg_ind]
        seg_counts = 0

        seg_confidences = np.zeros(len(columns))
        seg_abg_confidences = np.zeros(len(abg_pts))
        clusters, clusters_idx_groups = npi.group_by(abg_pts[:, -1], np.arange(len(abg_pts)))

        column_min_z = np.min(columns[:, 2])
        column_max_z = np.max(columns[:, 2])

        column_min_intensity = np.min(columns[:, 3])
        column_max_intensity = np.max(columns[:, 3])
        for clusters_idx in clusters_idx_groups:
            segs_pos = abg_pts[clusters_idx, :]
            segs_centroid = np.mean(segs_pos[:, :2], axis=0)
            centroid_distances = np.linalg.norm(segs_pos[:, :2] - segs_centroid, axis=1)
            if len(segs_pos) < 3:
                continue
            norm_intensity = (segs_pos[:, 3] - column_min_intensity) / (column_max_intensity - column_min_intensity)
            norm_z = (segs_pos[:, 2] - column_min_z) / (column_max_z - column_min_z)

            norm_z = np.exp(-(norm_z - 1) ** 2 * 5)
            norm_distance = np.exp(-(centroid_distances) ** 2 * 5)

            segs_peak_i = np.argmax(norm_z + norm_intensity + norm_distance)
            segs_centroid = segs_pos[segs_peak_i, :2]

            segs_vec = segs_pos[:, :2] - segs_centroid
            segs_prj = np.linalg.norm(segs_vec, axis=1)
            seg_confidence = gau_interp3D(segs_prj, band_width=gau_bandwidth)
            seg_abg_confidences[clusters_idx] = (seg_confidence - np.min(seg_confidence)) / (np.max(seg_confidence) - np.min(seg_confidence))
            seg_confidences[abg_ind] = seg_abg_confidences
            seg_counts += 1
            total_seg_counts += 1

        las.add_extra_dim(laspy.ExtraBytesParams(name="conf", type="float32", description="confidence"))
        las.conf = seg_confidences
        las.write(os.path.join(data_dir, pcd_fname + "_cfd.laz"))
        print("seg counts(%s): %d" % (pcd_fname, seg_counts))

    print("total seg counts: %d" % total_seg_counts)
    return True

def prepare_confidence(configs):
    ...
    pcd_fnames = list(set(train_pcd_fnames) | set(test_pcd_fnames) | set(valid_pcd_fnames))

    pcd_fnames = [pcd_fname for pcd_fname in pcd_fnames if pcd_fname]
    assign_confidence(configs,pcd_fnames)
...
def test(configs):
    ...
    for i, (nb_idxs, nb_vals, nb_anns, nb_pcd_idxs,nb_inverse_idxs, fname) in enumerate(test_generator):
        ...
        pcd_ann_pred = np.zeros([len(pcd),2],dtype=np.int32)
        for k, idx in enumerate(nb_idxs):
            x = torch.zeros(nb_tsz, attr_ct)
            if len(idx) > 0:
                x[idx, :] = torch.from_numpy(nb_vals[k].astype(np.float32))
        ...
        accu = voxelAccuracy(pcd_ann_pred[:, 1], pcd[:,-1].astype(np.int32))
        ious = classAccuracy(pcd_ann_pred[:, 1], pcd[:,-1].astype(np.int32), num_classes)
        ious = ious[1:]  # exclude empty cell class
        ...
        if myLogger is not None:
            myLogger.write("%s ---> Start testing %s (OA, IoU[other], IoU[conf], mIoU):\t" % (datetime.datetime.now(), fname))
    ...
    for k, pcd_ann_pred in enumerate(pcd_ann_preds):
        ...
        las.add_extra_dim(laspy.ExtraBytesParams(name="ConfPred", type="int32", description="confidence prediction"))
        las.add_extra_dim(laspy.ExtraBytesParams(name="ConfRef", type="int32", description="confidence reference"))

        abg_ind = las.VegCls > 1
        las.ConfRef[abg_ind]=pcd_ann_pred[:,0]
        las.ConfPred[abg_ind]=pcd_ann_pred[:,1]

        las.write(os.path.join(logs_dir, "test_{}.laz".format(test_generator.pcd_fnames[k])))
...
def apply(configs):
    ...
    for i, (nb_idxs, nb_vals, nb_anns, nb_pcd_idxs,nb_inverse_idxs, fname) in enumerate(app_generator):
        ...
        las.add_extra_dim(laspy.ExtraBytesParams(name="ConfPred",type="int32",description="confidence prediction"))
        las.ConfPred[las.VegCls > 1] = pcd_ann_pred
        las.write(os.path.join(logs_dir, "{}.laz".format(app_generator.pcd_fnames[i])))
        if myLogger is not None:
